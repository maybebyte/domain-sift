...
subtest 'argument parsing' => sub {
    subtest '-h displays usage' => sub {
        ...
        like($stderr, qr/domain-sift.*\[-h\].*\[-f\s+format\]/i,
            'usage message shown');
...
subtest 'output formats' => sub {
    ...
    subtest 'unbound format' => sub {
        ...
        is($exit, 0, 'exits successfully');
        like($stdout, qr/^local-zone: "example\.com" always_refuse$/m,
            'unbound format for example.com');
        like($stdout, qr/^local-zone: "test\.org" always_refuse$/m,
    ...
    subtest 'rpz format' => sub {
        ...
        is($exit, 0, 'exits successfully');
        like($stdout, qr/^example\.com CNAME \.$/m, 'rpz CNAME for example.com');
        like($stdout, qr/^\*\.example\.com CNAME \.$/m, 'rpz wildcard for example.com');
        like($stdout, qr/^test\.org CNAME \.$/m, 'rpz CNAME for test.org');
        like($stdout, qr/^\*\.test\.org CNAME \.$/m, 'rpz wildcard for test.org');
    };

    subtest 'rpz format reduces subdomains' => sub {
        my $file_with_subs = temp_file("example.com\nsub.example.com\ndeep.sub.example.com\n");
        my ($exit, $stdout, $stderr) = run_cli('-f', 'rpz', $file_with_subs);
        is($exit, 0, 'exits successfully');
        like($stdout, qr/^example\.com CNAME \.$/m, 'root domain present');
        unlike($stdout, qr/^sub\.example\.com CNAME \./m, 'subdomain removed');
        unlike($stdout, qr/^deep\.sub\.example\.com CNAME \./m, 'deep subdomain removed');
    };
};
...
subtest 'input handling' => sub {
    ...
    subtest 'missing file produces error' => sub {
        my ($exit, $stdout, $stderr) = run_cli('/nonexistent/file/path.txt');
        # The <<>> operator with autodie warns but continues, exiting 0
        like($stderr, qr/can't open|no such file/i, 'error message shown');
    };
...
subtest 'deduplication and sorting' => sub {
    ...
    subtest 'output is alphabetically sorted' => sub {
        ...
        is($exit, 0, 'exits successfully');
        my @lines = grep { /\S/ } split /\n/, $stdout;
        is_deeply(\@lines, [qw(apple.com mango.com zebra.com)], 'sorted output');
    };
    ...
    subtest 'duplicates across files deduplicated' => sub {
        ...
        is_deeply(\@lines, [qw(example.com other.net test.org)], 'deduplicated and sorted');
    };
};
...
subtest 'edge cases' => sub {
    subtest 'hosts file format' => sub {
        my $file = temp_file("127.0.0.1 localhost.com\n0.0.0.0 blocked.org\n");
        my ($exit, $stdout, $stderr) = run_cli($file);
        is($exit, 0, 'exits successfully');
        like($stdout, qr/^localhost\.com$/m, 'extracts domain after 127.0.0.1');
        like($stdout, qr/^blocked\.org$/m, 'extracts domain after 0.0.0.0');
    };

    subtest 'comment lines ignored' => sub {
        my $file = temp_file("# This is a comment\nexample.com\n# Another comment\n");
        my ($exit, $stdout, $stderr) = run_cli($file);
        is($exit, 0, 'exits successfully');
        my @lines = grep { /\S/ } split /\n/, $stdout;
    ...
    subtest 'mixed valid and invalid domains' => sub {
        my $file = temp_file("valid.com\ninvalid\nalso-valid.org\nno-tld\n");
        my ($exit, $stdout, $stderr) = run_cli($file);
        is($exit, 0, 'exits successfully');
        my @lines = grep { /\S/ } split /\n/, $stdout;
        is(scalar @lines, 2, 'only valid domains');
        is_deeply(\@lines, [qw(also-valid.org valid.com)], 'valid domains sorted');
    };
};
